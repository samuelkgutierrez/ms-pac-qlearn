...
{
	/* Some extra variables addressing then needs of CS 427 / CS 527 */
	public HashSet<GHOST> activeGhosts;
	public boolean endWhenPillsGone;
	public boolean pacmanWin;
	...
	public void setupForTrials()
	{
		List<Integer> ghostnodes = destroyMostPills(15);
		ghosts.get(GHOST.BLINKY).currentNodeIndex = ghostnodes.get(0);
		ghosts.get(GHOST.BLINKY).lairTime = 0;
		ghosts.get(GHOST.PINKY).currentNodeIndex = ghostnodes.get(1);
		ghosts.get(GHOST.PINKY).lairTime = 0;
		ghosts.get(GHOST.INKY).currentNodeIndex = ghostnodes.get(2);
		ghosts.get(GHOST.INKY).lairTime = 0;
		ghosts.get(GHOST.SUE).currentNodeIndex = ghostnodes.get(3);
		ghosts.get(GHOST.SUE).lairTime = 0;
		powerPills.clear(); /* Just get rid of all the power pills. */
	}
	...
	{
		endWhenPillsGone=false;
		pacmanWin=false;
		HashSet<GHOST> activeGhosts = new HashSet<GHOST>();
		activeGhosts.add(GHOST.INKY);
		activeGhosts.add(GHOST.BLINKY);
		activeGhosts.add(GHOST.PINKY);
		activeGhosts.add(GHOST.SUE);
	...
	/**
	 * destroyAllButTwoPills
	 * 
	 * Weird implementation involving pacman teleporting.
	 * The straightforward approach of iterating over pills didn't work right.
	 * (I still don't know why the straightforward approach doesn't work.)
	 */
	public List<Integer> destroyMostPills(int remaining)
	{
		int pac_initial_index = pacman.currentNodeIndex; /* Save pacman. */
		int current_lives = pacman.numberOfLivesRemaining;
		
		int[] active_pills_primitive = getActivePillsIndices();
		ArrayList<Integer> active_pills = new ArrayList<Integer>(active_pills_primitive.length);
		for(int x : active_pills_primitive) { active_pills.add(x); }
		
		Collections.shuffle(active_pills, rnd);

		for(int i = remaining; i < active_pills.size(); ++i)
		{
			pills.clear(getPillIndex(active_pills.get(i)));
		}
		
		pacman.currentNodeIndex = pac_initial_index; /* Restore pacman. */
		pacman.numberOfLivesRemaining = current_lives;
		score = 0; /* Don't give points for the destroyed pills. */
		
		List<Integer> ghostnodes =
				active_pills.subList(active_pills.size() - 4, active_pills.size());
		return ghostnodes;
	}
	...
	private void _levelReset()
	{
		...
		if(pacman.numberOfLivesRemaining % 2 == 0)
		{
			pacman.currentNodeIndex=currentMaze.initialPacManNodeIndex;			
		}
		else
		{
			pacman.currentNodeIndex=getGhostInitialNodeIndex();
		}
	...
	{	
		for(Ghost ghost : ghosts.values())
		{
			if(ghost.lairTime>0)
			{
				// a hack that should keep inactive ghosts in the lair)
				GHOST ghost_type = ghost.type;
				if(!activeGhosts.contains(ghost_type)) {continue;}
	...
	{
		pacman.numberOfLivesRemaining = lives;
	}
